...
class MazeGUI:
    ...
    def navigate(self, choice):
        if choice == 'b':
            if len(self.game.visited_nodes) >= 2:
                self.game.visited_nodes.pop()  # remove current node
                self.current_node = self.game.visited_nodes.pop()  # set previous node as current
                self.update_prompt()
            else:
            ...
            if choice in ['l', 'r']:
                ...
                if next_node:
                    self.game.visited_nodes.append(self.current_node)
